"""Smoke test for the xMatters MCP server.

Connects to the running server over streamable HTTP (proving transport and
bearer auth work) and calls list_groups().

- With no xMatters credentials configured, the server must return the graceful
...
async def run() -> int:
    ...
    if not settings.credentials_present:
        if isinstance(data, dict) and data.get("error") == NOT_CONFIGURED_ERROR["error"]:
            print("PASS list_groups: graceful not-configured error returned")
            print("     transport + auth verified without xMatters credentials")
            return 0
        print(f"FAIL list_groups: expected {NOT_CONFIGURED_ERROR!r}, got {data!r}")
        return 1

    if isinstance(data, (list, dict)):
        summary = " ".join(str(data).split())[:140]
        print(f"PASS list_groups: {summary}")
        return 0
    print(f"FAIL list_groups: unexpected response {data!r}")
    return 1
